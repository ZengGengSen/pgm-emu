...
OPED(0x4f): // LD   R,A
	...
	zR = zA;
#else
	zR = zA - ((Z80->nCyclesSegment - Z80->nCyclesLeft) >> 2);
#endif
	zR2 = zA & 0x80;
	RET(5)
...
OPED(0x5f): // LD   A,R
	...
	zA = (zR & 0x7f) | zR2;
#else
	zA = ((zR + ((Z80->nCyclesSegment - Z80->nCyclesLeft) >> 2)) & 0x7f) | zR2;
#endif
	zF = (zF & CF) | SZ[zA] | zIFF2;
	RET(5)
...
OPED(0xb1): // CPIR
	do
	{
		...
		zF = F;
		USE_CYCLES(17)
	} while (zBC && !(F & ZF) && (Z80->nCyclesLeft > 0));
	goto OP_CPXR;
